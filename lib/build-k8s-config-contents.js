...
module.exports = async function (namespace, disableBuild, branchPath = 'development') {
  ...
  if (!cellInfo.dna[branchPath]) throw new Error(`${branchPath} dna needed for cell ${packagejson.name}`)
  let registry
  let imageTag
  let k8sConfig = cellInfo.dna[branchPath]
  if (!disableBuild) {
